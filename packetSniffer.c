...
struct Dns {
	...
	u_char dns_qr 		/* (1 bit) Query Response Flag, differentiate between queries and responses [0=Query] [1=Response] */
	
	// define Opcode flags
	u_char dns_opcode;	/* (4 bits) Operation Code,  any other number = Unassigned	*/
	#define DNS_SQ	0x00	/* Standard Query 			*/  
	#define DNS_IQ 	0x01	/* Inverse Query, 	(now obsolete, lookp name from ip addr)			*/ 
	#define DNS_SR	0x02	/* Status request,	(server status request)		*/
	#define DNS_NOT	0x04	/* Notify, (primary server tell secondary servers for zone transfer)	*/
	#define DNS_UPT	0x05	/* Update, (Allow implement Dynamic DNS, add & delete of records selected) */
	
	u_char dns_aa;		/* (1 bit) Authoritative Answer Flag, show if server that created response is authoritative for the zone */
	u_char dns_tc;  	/* (1 bit) Truncation Flag, show that msg was truncated due to being too long */
	...
	u_char dns_z;		/* (1 bits) Zero, 1 reserved bit set to '0' */
	...
	#define DNS_NOERR 0x00	/* No Error, no error occ */
	#define DNS_FERR  0x01	/* Format Error, server unable to respond to query due to how query was constructed */
	#define DNS_SFAIL 0x02	/* Server Failure, server unable to respond due to server itself */
	#define DNS_NERR  0x03	/* Name Error, name specified does not exist in domain */
	#define DNS_NIMPL 0x04	/* Not Implemented, type of query received not supported */
	#define DNS_REF	  0x05	/* Refused, server refused query process due to policy or technical reasons */
	#define DNS_YXDOM 0x06	/* YX Domain, name exists where it should not */
	#define DNS_YXRRS 0x07	/* YX RR Set, resource record exists where it should not */
	#define DNS_NXRRS 0x08	/* NX RR Set, resource record that should exist does not */
	#define DNS NAUTH 0x09	/* Not Auth, server receiving query is not authoritative for zone specified */
	#define DNS_NZ	  0x0A	/* Not Zone, name speicified is not within the zone specified in msg */
...
void print_dns(u_char *args, const struct pcap_pkthdr *hdr, const u_char *packet,char * protocol,const struct Ip *ip,const struct udphdr *udph,FILE *f){
...
void print_dns(u_char *args, const struct pcap_pkthdr *hdr, const u_char *packet,char * protocol,const struct Ip *ip,const struct Tcp *tcp,FILE *f){
	...
	switch(ip->ip_p) {
		case IPPROTO_TCP:
			...
			if(ntohs(tcp->th_sport) == 53 || ntohs(tcp->th_dport) == 53){
				print_dns(args,hdr,packet,protocol,ip,tcp,f);
			}else{// Else
		...
		case IPPROTO_UDP:
			...
			if(ntohs(udph->source) == 53 || ntohs(udph->dest) == 53){
				print_dns(args,hdr,packet,protocol,ip,udph,f);
			}else{// else print_udp
